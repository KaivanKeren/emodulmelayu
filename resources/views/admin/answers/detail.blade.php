            ...
            <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-500">Detail jawaban untuk {{ $user->name }}</p>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div class="p-4 bg-blue-50 rounded-lg">
                        ...
                        <p class="text-2xl font-bold text-blue-800">{{ $summary['total_score'] }}%</p>
                    </div>
                    <div class="p-4 bg-green-50 rounded-lg">
                        <p class="text-sm text-green-600 font-medium">Penyelesaian</p>
                        <p class="text-2xl font-bold text-green-800">{{ $summary['completion_percentage'] }}%</p>
                    </div>
                    <div class="p-4 bg-purple-50 rounded-lg">
                        <p class="text-sm text-purple-600 font-medium">Pertanyaan Dijawab</p>
                        <p class="text-2xl font-bold text-purple-800">
                            {{ $summary['answered_questions'] }}/{{ $summary['total_questions'] }}</p>
                    </div>
                    <div class="p-4 bg-orange-50 rounded-lg">
                        <p class="text-sm text-orange-600 font-medium">Waktu Diperlukan</p>
                        <p class="text-2xl font-bold text-orange-800">
                            @if ($summary['assessment_completed_at'])
                                {{ Carbon\Carbon::parse($summary['assessment_started_at'])->diffForHumans($summary['assessment_completed_at'], true) }}
                            @else
                                Dalam Proses
                            @endif
                        </p>
                        ...
                        <div
                            ...
                            <div class="flex items-start justify-between">
                                <div class="flex-1">
                                    ...
                                    <h3 class="ql-viewer mt-2 text-lg font-medium text-gray-900">{!! $detail['question']['content'] !!}
                                    </h3>
                                    <p class="mt-1 text-sm text-gray-500">
                                        Tipe Soal: @if ($detail['question']['type'] === 'multiple_choice')
                                            Pilihan Ganda Kompleks
                                        @elseif ($detail['question']['type'] === 'single_choice')
                                            Pilihan Ganda
                                        @endif
                                    </p>
                                </div>
                            ...
                            <div class="mt-4 space-y-2">
                                @foreach ($detail['all_options'] as $option)
                                    <div
                                        ...
                                        <!-- Option Text -->
                                        <span
                                            class="ql-viewer text-sm {{ $option['is_correct'] ? 'font-medium' : 'text-gray-700' }}">
                                            {!! $option['content'] !!}
                                        </span>
                                    </div>
                                @endforeach
